...
//
// Still in some cases, a node.js API method references a "listener". That is
...
l8.proto.require.promise = function( path ){
// Like native require() but returns a promise instead.
...
// node.js signal an error.
  return function(){
    ...
    var cb = args[ args.length ] = l8.flow // Like l8.walk but detect exceptions
    try{
...
function l8_require( path ){
  ...
  // Adapt one or more functions, classic nodejs cb( err, rslt ) style
    ...
    var l8ized   = l8ize( async )
    l8ized.go    = l8goize( async )
    l8ized.async = async
    l8ized.sync  = sync
    async.l8     = l8ized
    async.go     = l8ized.go
  ...
  // to use a XXX.new(... ) instead.
    m[ klass.name + ".new" ] = function(){
  ...
  switch( path ){
    ...
    case "buffer": // http://nodejs.org/api/buffer.html
      global.Buffer = adapt_class( m.Buffer )
    break
