...
function MakeActorConstructor( name, pattern, options ){
  return function(){
...
ProtoStage.setConnectionHandlers = function(){
  ...
  conn.on( 'send', function( msg ){
    de&&bug( ["'send' socketIo event from " + that].concat( msg))
    var actor = ProtoActor.lookup( msg.name)
    if( !actor ){
      de&&bug( "'send' socketIo event for unknown " + msg.name + " actor")
      return
    }
    actor.send( {remote:that,message:msg.send} )
  })
  conn.on( 'call', function( msg ){
    de&&bug( ["'call' socketIo event from " + that].concat( msg))
    var actor = ProtoActor.lookup( msg.name)
    if( !actor ){
...
ProtoProxyActor.call = function( args, caller ){
  ...
  this.stage.then(
    function( conn ){
      ...
      de&&bug(
        "'call' number " + cb_id + " on " + that.stage + " for " + that.name,
        args
      )
