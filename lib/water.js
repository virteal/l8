...
(function(){
  ...
  function delivery( h2o, destination ){
    ...
    return destination._deliverer = function(){
      ...
      if( !slot ){
        de&&bug( "Visit, ctx:", h2o.id, "destination:", destination.id, "args:", arguments[0] );
        queue.push( slot = { water: h2o, destination: destination, args: arguments } );
      // If another call, just override previous arguments
      }else{
        de&&bug( "Revisit, ctx:", h2o.id, "destination:", destination.id, "args:", arguments[0] );
        slot.args = arguments;
      }
    };
  }
  ...
  function dispatch(){
    ...
    while( count < list.length  ){
      ...
      while( slot = list[ ii++ ] ){
        ...
        de&&bug( "Deliver change on water", slot.destination.id );
        ...
        while( source = list[ jj++ ] ){
          ...
          de&&mand( destination._water );
          if( depends_on( destination, source ) )break;
        }
        if( source )break;
  ...
  water.filler = function( x, dflt ){ return x( dflt, water ) };
  ...
  Fluid.prototype = {
    ...
    flatmap: function( p ){
      ...
      var f = this.make( water( this._water, function( v ){
        if( v && v.water )return v;
        fluid.it = f;
        var result = p.call( water.it = ctx, v, index++, f );
        if( result && result.water )return result;
        if( result && result.forEach && !result._water ){
    ...
    route: function( ways, f ){
      ...
      var r;
      var err = this.make( water( water, function( v ){
        if( v && v.water )return v;
        if( p ){
          // Two ways output
          r =  try_call( f, fluid.it = err, v );
          if( r ){
  ...
  function MakeFluid( source ){
    var f = new Fluid();
    f.fluid = Fluid();
    return f.factory.apply( fluid.it = f, arguments );
  }
  ...
  MakeFluid.define = function( name, fn ){
  ...
  if( 1 ){
    ...
    fact_loop(  fact, function( v ){
      ...
      if( n <= 1 ){
        ...
        // trace( "DEBUGGER" );
        return [ n - 1, n * total ];
        // fact_loop(  [ n - 1, n * total ] );
        // water.effect( function(){ fact_loop( [ n - 1, n * total ] ) } );
      }
    });
    fact_loop( fact_loop );
    ...
    water.fluid.define( function square(){
