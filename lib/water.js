...
(function(){
  ...
  function pump( source, sink, value ){
    pumps.push( { source: source, sink: sink, value: value } );
    return source;
  }
  ...
  function dispatch(){
    ...
    if( !queue.length ){
      ...
      while( item = list[ ii++ ] ){
        delivery( item.source, item.sink )( item.value );
      }
    }
  ...
  if( 1 ){
    ...
    // Diamond DAG, without unecessary recomputations
    // A, B, C, D where D = B + C, B = A * 5 and C = A * 5 
    var A = water( 0 );
    var B = water();
    var C = water();
    var D = water();
    ...
    // This is "Turing Complete" thanks to water.pump()
    ...
    fact_loop(  fact, function( v ){
      ...
      if( n <= 1 ){
        ...
        water.pump( fact_loop, fact_loop, [ n - 1, n * total ] );
      }
    });
