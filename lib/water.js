...
(function(){
  ...
  function delivery( machine, h2o, destination ){
    ...
    // Return a function that will update the allocated scheduled slot
    return destination._deliverer = function(){
      // If called for the first time
      if( !slot ){
        de&&bug( "Visit, ctx:", inspect( h2o ),
        ...
        machine.queue.push( slot = { water: h2o, destination: destination, args: arguments } );
      // If another call, just override previous arguments, unless queued
      }else{
        de&&bug( "Revisit, ctx:", inspect( h2o ),
        "destination:", inspect( destination ),
        "args:", 
        (arguments[0] && arguments[0].water)
        ? "Water.error: " + arguments[0].error
  ...
  function dispatch( machine ){
    ...
    while( count < list.length  ){
      ...
      // If nothing was done, there is an issue
      if( !done ){
  ...
  function try_call( f, target, arg1, arg2 ){
    ...
    }catch( err ){ return water.failure( err ); }
  }
  ...
  // ToDo: move outside of closure, using ._current
  ...
  water.machine = function(){ return new Machine(); };
  ...
  if( 1 ){
    ...
    .log( "last" );
    err_fluid.push( "ok" ).fail( "some error" ).close(); // => ok default
  }
