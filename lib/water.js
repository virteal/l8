...
(function(){
  ...
  function dispatch(){
    ...
    water.round++;
    de&&bug( "Deliver list", list.length );
    while( slot = list[ ii++ ] ){
      de&&bug( "Deliver", slot.water.id );
    ...
    var h2o = function( val, auto ){
     ...
     if( val === water ){
        ...
        if( arguments.length === 2 && sources ){
          var f  = auto.boxon || auto;
          var f2 = function( v ){ f( (v && v.water) ? v.err : null, v ); };
  ...
  water.water   = function( v ){ return (w && w._water) || void 0; };
  ...
  Fluid.prototype = {
    ...
    filter: function( p ){
      return new Fluid( water( water, function( v ){
        if( v.water )return;
        if( p( v ) )return v;
      }, this._water ) );
    },
    ...
    reduce: function( p, init ){
      if( arguments.length === 1 ){ init = water; }
      return new Fluid( water( init, function( p, v ){
        if( v.water )return;
        return p( this(), v );
      }, this._water ) );
    },
