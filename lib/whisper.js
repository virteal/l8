...
var Parole = (function(){
  ...
  P.defer = defer; function defer(){
    ...
    p.resolve          = function( v ){ p.resolve(  v ); };
    p.reject           = function( r ){ p.reject(  v ); };
    p.notify           = function(){ apply( fill, p, arguments ); };
    p.makeNodeResolver = function(){ apply( makeNodeResolver, p, arguments );};
    return p.promise = p;
  }
  ...
  P._tail = _tail; function _tail(){
    ...
    step = this.next;
    while( step._step ){ step = step.next; }
  ...
  P.define = define; function define( f ){
    return function(){
      var gen = parole();
      f.apply( gen, [ out ].concat( array_slice.call( arguments, 0, -1 ) ) );
      var idx_cb = arguments.length - 1;
      gen._tail().on( arguments[ idx_cb ] );
    };
  }
  ...
  P.generator = generator; function generator( f ){
    return function(){
      ...
      if( !arguments.length ){
        ...
        f.apply( gen, concat( slice1( arguments ) ) );
      }
      return function(){
        var idx_cb = arguments.length - 1;
        gen._tail()._listener = arguments[ idx_cb ];
        fill.apply( gen._in, array_slice.call( arguments, 0, -1 ) );
      };
    };
  }
  ...
  P.when = when; function when( stuff ){
    ...
    if( arguments.length > 1 ){
      ...
      stuff.then(
        function(){ apply( resolve, p, arguments ); },
        function(){ apply( reject,  p, arguments ); }
      );
    }else{
      p.fill( null, stuff );
    }
  ...
  P._join = _join; function _join(){
    ...
    if( !(fork._fork && fork._step) ){
      if( !fork.previous || fork.previous === P ){ fork = this._tail(); }
  ...
  P.join = join; function join(){
    if( !this
    || (!this.promises && (!this.previous || this.previous === P ))
    ){
  ...
  P.upgrade = upgrade; function upgrade( v, delay ){
    if( arguments.length > 1 ){
      var that = this;
      var args = arguments;
      setTimeout( function(){ apply( resolve, that, args ); }, delay );
    }
  ...
  P.fill = fill; function fill( p ){
    ...
    if( this._step ){
      if( p && typeof p.then === "function" &&  arguments.length === 1 ){
        ...
        this.values = arguments;
        try{
          this._step.apply( this.next, arguments );
        }catch( err ){
          var next = this.next;
