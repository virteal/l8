...
// If you read this code, please read the doc in the github wiki too because
// there is some level of DRY comments here.
...
(function(){
  ...
  function MakeParole(){
    // Create a new function, when called it just calls itself.fill(...)
    // eqv var p = fill.bind( p ) but without the self reference issue
  ...
  // Without __proto__ the API runs slower but still works, feature detection
  ...
  if( typeof Promise !== "undefined" ){
    ...
    EcmaLikePromise = function( f ){
      var p = MakeParole();
      f( bind( resolve, p ), bind( reject, p ));
      return p;
    };
  }
  ...
  if( typeof require === "undefined" ){
    if( typeof Boxon !== "undefined" ){
      P.boxon = Boxon.scope( parole );
    }
  }else{
    P.boxon = require( "l8/lib/boxon.js").scope( parole );
  }
  I.boxon = function(){
    var b = P.boxon.apply( null, arguments);
    _on( this, b );
    return b;
  };
  ...
  function from( step ){
    ...
    // p.from( pipe ) => pipe.pipe( p )
    if( arguments.length === 1 && isPipe( step ) ){
      step.pipe( this );
      return this;
    }
    // p.from( water ) => water( p )
    if( arguments.length === 1 && step && step._water ){
      step._water( this );
      return this;
    }
  ...
  function when( stuff ){
    ...
    }else if( stuff && typeof (then = stuff.then) === "function" ){
      then.call( stuff,
    ...
    && typeof stuff.next     === "function" 
    && typeof stuff["throw"] === "function"
    ){
      var next = function( v ){
        var r;
        try{ r = stuff.next( v ); }catch( e ){ return p.reject( e ); }
        if( r.done ){
          return p.resolve( r.value );
        }else{
          return p.resolve( r.value ).then(
            next,
            function( e ){ stuff["throw"]( e ); }
          );
        }
      };
      return next();
        ...
        _on(
          ...
          bind( function( ii, value ){
            ...
            if( ++count === len ){
              ...
              fill.call( parole.current = next, parole.next = next._next, list );
            }
          }, branch, ii )
        );
      }
  ...
  function resolve( v ){
    ...
    if( !isParole( p ) ){ p = MakeParole(); }
    var then
    if( false && v && typeof (then = v.then) === "function" ){
      then.call( v,
        bind0( resolve, p ),
        bind0( reject,  p )
      );
  ...
  function reject( r ){
    ...
    if( !isParole( p ) ){ p = MakeParole(); }
    var then;
    if( false && r && typeof (then = r.then) === "function" ){
      then.call( r,
        bind0( reject, p ),
        bind0( reject, p )
      );
  ...
  function _progress( target ){
    ...
    // If value is a thenable, wait until delivery, unless error
    var thenable = target.value;
    var done = false;
    if( !target.error
    && thenable
    && (typeof thenable === "object" || typeof thenable === "function" )
    ){
      try{
        var then = thenable.then;
        if( typeof then === "function" ){
          if( thenable === target )throw new TypeError( "Resolve to itself" );
          var redo_ok = function( value ){
            if( done || target.scheduled )return;
            if( target.error ){
              target.values[ 0 ] = reason( value );
            }else{
          ...
          try{
            then.call( thenable, redo_ok, redo_ko );
          }catch( err ){
            redo_ko( err );
          }
          return;
  ...
  function _resolver(){
    ...
    try{
      // As per promise/a spec, 'this' must be null
      // ToDo: change doc
      var rslt = fn( this.value );
      var then;
      if( !( next = this._next ) ) return;
      if( !rslt
      || !( typeof rslt === "object" || typeof rslt === "function" )
      || typeof (then = rslt.then) !== "function"
      ){
        next.fill( null, rslt );
        return;
      }
      // Cannot resolve to itself
      if( rslt === next )throw new TypeError( "Resolve to itself" )
      then.call( rslt,
  ...
  function each( handler, objects ){
    ...
    var ii = 0, len = p.promises.length; while( ii < len ){
      a_p = p.promises[ ii ];
      if( a_p && typeof (then = a_p.then) === "function" ){
        then.call( a_p,
  ...
  */
  ...
  // Install default scheduler (based on setImmediate() idealy)
  scheduler();
