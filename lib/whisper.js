...
var Parole = (function(){
  ...
  // Without __proto__ the API runs much slower, but it works.
  ...
  var _set_proto = function( obj ){ obj.__proto__ = P; }
  ...
  P.parole = I.parole = parole; function parole(){
    ...
    return !arguments.length ? p : when.apply( p, arguments );
  }
  ...
  P.defer = defer; function defer(){
    // ToDo: optimize this, Q compatible instance don't need to be functions
    ...
    p.reject           = function( r ){ fill.call(  p, v ); };
    p.notify           = function(){ emit.apply( p, arguments ); };
  ...
  P.resolve = I.resolve = IQ.resolve = resolve; function resolve( v ){
    var p = this;
    if( !isParole( p ) ){ p = parole(); }
  ...
  P.reject = I.reject = IQ.reject = reject; function reject( r ){
    var p = this;
    if( !isParole( p ) ){ p = parole(); }
  ...
  // Q's
  IQ.makeNodeResolver = makeNodeResolver; function makeNodeResolver(){
    var that = this;
  ...
  if( false && has_proto ){
    ...
    src += "\n}";
    console.log( "_set_proto compilled to: " + src );
