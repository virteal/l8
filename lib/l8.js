 ...
 */
 ...
 var DEBUG = true // (typeof window === 'undefined')
...
// will use. That returned function must have the signature of console.log()
var Log = function(){ return (console && console.log) || (Util && Util.puts) || console.log }

// User can redefine log using l8.logger(). It must be a function returning a
// function to be called to log a message. l8.trace() will use it. To restore
// it to its default value, use l8.logger( null)
// example: l8.logger( function(){ return console.log } }
// Note: because log is a function that return the log function, ie because
// of this added level of indirection, use can fine tune logging using
// information available in the context of the application when the log
// function is necessary, versus the information available when the log
// is registered.
var log = Log
...
ProtoTask.init = function( parent, is_fork, is_spawn ){
  this.current          = this // root task l8 defines it differently
...
// steps are blocked too.
  // ToDo: fix that, ie have a true step_init( step, task, ... )
...
l8.timeNow = null
l8.dateNow = null
ProtoTask.__defineGetter__( "now", function(){ return l8.timeNow } )
...
var L8_NextTick = ProtoTask.tick = l8.nextTick
...
ProtoTask.logger  = function( l ){
  var previous = log
  arguments.length && (log = l || Log )
  return previous
}

l8.serverize = function(){
  if( !Util )throw new Error( "Cannot l8.serialze(), not a server")
  l8.server = true
  l8.client = false
}
...
l8.client = !Util
l8.server = Util
l8.getDebugFlag = function(){ return de }
l8.__defineGetter__( "de", l8.getDebugFlag)
...
var L8_EnqueueStep = function( step ){
  ...
  de&&mand( !step.task.stepResult || !step.task.stepResult.then || step.task.stepResult.parentTask )
  L8_NextTick(
...
ProtoTask.callback = function( promise, cb ){
...
ProtoPort.signal = ProtoPort.tryPut
...
ProtoMessageQueue.signal = ProtoMessageQueue.tryPut
...
ProtoGenerator.signal = ProtoGenerator.tryYield
